...
@section('title', 'Dashboard')
              ...
              <div class="grid grid-cols-1 xl:gap-4 my-4">                    
                  <div class="flex flex-col gap-6 bg-white shadow rounded-lg p-4 sm:p-6 xl:p-8 ">
                    <div class="flex justify-between items-center">
                      <h3 class="text-xl leading-none font-bold text-gray-900">Daftar Penerbit</h3>
                      <button id="openModal" onClick="changeAction('/tambah-penerbit')" class="ml-5 text-white bg-cyan-600 hover:bg-cyan-700 focus:ring-4 focus:ring-cyan-200 font-medium rounded-lg text-sm px-5 py-2.5 text-center items-center mr-3">Tambah Pembeli</button>
                    </div>
                            ...
                            @foreach($data as $d)
                              <tr class="text-gray-500">
                                ...
                                <th class="border-t-0 px-4 align-middle text-sm font-normal whitespace-nowrap p-4 text-left flex gap-2">
                                  <a href="#!" id="openModalEdit" class="text-green-500 edit-button" data-nama="{{ $d->nama }}" data-email="{{ $d->email }}" 
                                  data-alamat="{{ $d->alamat }}" onClick="changeAction('/edit-penerbit', {{ $d->id_penerbit }})">Edit</a> | 
                                  <a href="/hapus-penerbit/{{ $d->id_penerbit }}" class="text-red-400">Delete</a>
                                </th>
                              </tr>
                            @endforeach
                          </tbody>
  ...
  <div class="hidden py-12 bg-gray-700 bg-opacity-75 transition duration-150 ease-in-out absolute top-0 right-0 bottom-0 left-0 z-50" id="modal">
      <div role="alert" class="container mx-auto w-11/12 md:w-2/3 max-w-lg">
          <div class="relative py-8 px-5 md:px-10 bg-white shadow-md rounded border border-gray-400">
            ...
            {{ csrf_field() }}
              <input id="id_penerbit" name="id_penerbit" type="hidden" />
              <h1 class="text-gray-800 font-lg font-bold tracking-normal leading-tight mb-4">Input Penerbit</h1>
  ...
  <script type="text/javascript">
    ...
    function changeAction(action, id_penerbit = null){
      $(':input.text-gray-600').val("");
      $('#my-form').attr('action', action);
      $('#id_penerbit').val(id_penerbit);
    }
